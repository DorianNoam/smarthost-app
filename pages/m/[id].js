...
export default function PropertyChat() {
  ...
  const fetchPropertyData = async () => {
    // On ne sélectionne QUE le nom pour l'affichage — aucune donnée sensible côté client
    ...
    if (data) {
      ...
      const greetings = {
        ...
        de: `Hallo! Ich bin Marc, Ihr Butler für Ihren Aufenthalt im "${data.name}". Wie kann ich Ihnen helfen?`,
        it: `Buongiorno! Sono Marc, il vostro maggiordomo per il vostro soggiorno a "${data.name}". Come posso aiutarvi?`
      };
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ 
          ...
          propertyId: property.id,   // ✅ On n'envoie que l'ID — le serveur charge les données
  ...
  return (
    <div className="app-container">
      <Head>
        {/* Page privée par logement — ne pas indexer */}
        <meta name="robots" content="noindex, nofollow" />
        <title>Conciergerie — {property.name}</title>
      </Head>
      <style jsx global>{`
      ...
      <style jsx>{`
        ...
        .send-btn { background: #1a2a6c; border: none; width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; }
      `}</style>
      ...
      <div className="input-bar">
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendMessage()} placeholder="Posez votre question..." />
