...
export default function MessageHistory() {
  ...
  const fetchMessages = async () => {
    // On récupère la ligne unique de conversation pour ce logement
    ...
    // Si on a des données et que la colonne history contient bien un tableau
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            <div key={index} className={`bubble ${msg.role === 'marc' ? 'marc' : 'guest'}`}>
              ...
              {msg.text}
              {/* On affiche l'heure à partir du timestamp stocké dans le JSON */}
              <span className="time">
