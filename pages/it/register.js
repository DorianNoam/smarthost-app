...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!email || !password || !firstName) { setError('Compila tutti i campi.'); return; }
    setLoading(true);
    setError(null);
    try {
  ...
  return (
    <div className="container">
      <Head>
        <title>Crea un account — Alfred Major | Maggiordomo IA per Affitti</title>
        <meta name="description" content="Crea il tuo account Alfred Major gratuitamente. Configura il tuo maggiordomo IA in 5 minuti. Primo mese gratis, senza impegno." />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://www.alfredmajor.com/it/register" />
        ...
        <link rel="alternate" hrefLang="it" href="https://www.alfredmajor.com/it/register" />
        <meta property="og:title" content="Crea un account — Alfred Major" />
      ...
      <div className="lang-switcher">
        ...
        <button className="lang-btn" onClick={() => router.push('/es/register')}>🇪🇸 ES</button>
      ...
      <div className="card">
        <div className="card-title">{inviteId ? 'Unisciti al team' : 'Crea un account'}</div>
        <div className="card-sub">{inviteId ? 'Crea il tuo account per accettare l\'invito.' : 'Unisciti agli host che si fidano di Alfred.'}</div>

        {inviteId && inviteReady && (
          <div className="invite-banner">
            <div className="invite-banner-title">✉️ Invito Alfred Major</div>
            <div className="invite-banner-sub">Crea il tuo account qui sotto per accettare.</div>
          </div>
        )}
        ...
        <form onSubmit={handleRegister}>
          <label>Nome</label>
          <input type="text" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Il tuo nome" autoCapitalize="words" />
          <label>Email</label>
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="8 caratteri minimo" />
          <button type="submit" className="btn" disabled={loading}>
            {loading ? 'Creazione in corso...' : inviteId ? '✅ Crea account & Unisciti' : 'Crea il mio account'}
          </button>
        </form>

        <div className="footer-link">
          Hai già un account? <Link href={inviteId ? `/it/login?invite=${inviteId}&email=${encodeURIComponent(email)}` : '/it/login'}>Accedi</Link>
        </div>
      </div>
    </div>
  );
