...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (authError) {
      setError('Credenciales inválidas o cuenta no confirmada.');
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="container">
      <Head>
        <title>Iniciar sesión — Alfred Major | Panel de Anfitrión</title>
        <meta name="description" content="Inicia sesión en tu panel de Alfred Major para gestionar tus propiedades y tu mayordomo IA." />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://www.alfredmajor.com/es/login" />
        ...
        <meta property="og:title" content="Iniciar sesión — Alfred Major" />
      ...
      <div className="card">
        <div className="card-title">{inviteId ? 'Unirse al equipo' : 'Iniciar sesión'}</div>
        <div className="card-sub">{inviteId ? 'Inicia sesión para aceptar la invitación.' : 'Bienvenido de nuevo.'}</div>

        {inviteId && (
          <div className="invite-banner">
            <div className="invite-banner-title">✉️ Invitación pendiente</div>
            <div className="invite-banner-sub">Inicia sesión para acceder al panel de tu equipo.</div>
          </div>
        )}
        ...
        <form onSubmit={handleLogin}>
          ...
          <Link href="/forgot-password" className="forgot">¿Olvidaste tu contraseña?</Link>
          <button type="submit" className="btn" disabled={loading}>
            {loading ? 'Iniciando sesión...' : inviteId ? '✅ Iniciar sesión & Unirse' : 'Iniciar sesión'}
          </button>
        </form>

        <div className="footer-link">
          ¿No tienes cuenta? <Link href={inviteId ? `/es/register?invite=${inviteId}&email=${encodeURIComponent(email)}` : '/es/register'}>Crear cuenta</Link>
        </div>
      </div>
    </div>
  );
