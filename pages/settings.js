...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    if (router.query.connect === 'success') {
      checkConnectStatus();
    }
  }, [router.query]);

  useEffect(() => {
    const handleFocus = () => { if (user) loadUserData(); };
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      setBilling({ address: saved.address || '', zipcode: saved.zipcode || '', city: saved.city || '' });
    } catch (_) {}
  ...
  const loadTeam = async (ownerId) => {
    setTeamLoading(true);
    const { data } = await supabase.from('team_members').select('*').eq('account_owner_id', ownerId).order('created_at', { ascending: false });
    setTeamMembers(data || []);
    setTeamLoading(false);
  };

  const loadProperties = async (ownerId) => {
    const { data } = await supabase.from('properties').select('id, name').eq('owner_id', ownerId).order('name');
    setProperties(data || []);
  };
  ...
  const connectConfig = {
    not_connected: { bg: '#f8fafc', border: '#e2e8f0', badge: { label: '⚡ Non connecté', color: '#64748b', bg: '#f1f5f9', border: '#e2e8f0' }, btnLabel: 'Connecter mon compte Stripe', btnColor: '#1a2a6c' },
    pending:       { bg: '#fffbeb', border: '#fde68a', badge: { label: '⏳ En cours de vérification', color: '#d97706', bg: '#fffbeb', border: '#fde68a' }, btnLabel: 'Continuer la configuration', btnColor: '#d97706' },
    active:        { bg: '#f0fdf4', border: '#bbf7d0', badge: { label: '✅ Compte actif', color: '#15803d', bg: '#f0fdf4', border: '#bbf7d0' }, btnLabel: 'Gérer mon compte Stripe', btnColor: '#15803d' },
  };
    ...
    <div className="dashboard-layout">
      ...
      <main>
        <div className="settings-container">
          ...
          {/* ── REVENUS & UPSELLS (Stripe Connect) ── */}
          <div className="settings-card">
            <h2>💰 Revenus & Upsells</h2>

            <p style={{ margin: '0 0 20px', fontSize: '14px', color: '#64748b', lineHeight: 1.6 }}>
              Connectez votre compte Stripe pour encaisser directement les services additionnels achetés par vos voyageurs (late check-out, pack romantique, transfert aéroport...). <strong style={{ color: '#1a2a6c' }}>0% de commission Alfred Major.</strong>
            </p>
            ...
            <div style={{ background: cc.bg, border: `2px solid ${cc.border}`, borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <h3 style={{ margin: '0 0 4px', fontSize: '16px', fontWeight: 800, color: '#1a2a6c' }}>Compte Stripe Connect</h3>
                  <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>
                    {connectStatus === 'active' ? 'Votre compte est prêt à encaisser des paiements.' :
                     connectStatus === 'pending' ? 'Vérification en cours — finalisez votre inscription Stripe.' :
                     'Connectez votre compte Stripe pour activer les upsells.'}
                  </p>
                </div>
                <span style={{ background: cc.badge.bg, color: cc.badge.color, border: `1px solid ${cc.badge.border}`, padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: 700, whiteSpace: 'nowrap' }}>
                  {cc.badge.label}
                </span>
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{
                    background: connectStatus === 'active' ? '#f1f5f9' : cc.btnColor,
                    color: connectStatus === 'active' ? '#1a2a6c' : 'white',
                    border: connectStatus === 'active' ? '1px solid #e2e8f0' : 'none',
                    padding: '12px 20px', borderRadius: '12px', fontWeight: 700,
                    fontSize: '14px', cursor: connectLoading ? 'not-allowed' : 'pointer',
                    opacity: connectLoading ? 0.6 : 1, fontFamily: 'inherit',
                  }}
                >
                ...
                {connectStatus === 'pending' && (
                  <button
                    ...
                    style={{ background: 'white', color: '#64748b', border: '1px solid #e2e8f0', padding: '12px 20px', borderRadius: '12px', fontWeight: 700, fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    {connectChecking ? '⏳ Vérification...' : '🔄 Actualiser le statut'}
                  </button>
                )}
              </div>
            </div>
            ...
            {connectStatus !== 'active' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
                {[
                  { emoji: '💶', title: 'Paiement direct', desc: "L'argent arrive sur votre compte Stripe" },
                  { emoji: '0️⃣', title: '0% de commission', desc: 'Alfred Major ne prend aucun frais' },
                  { emoji: '🔒', title: 'Paiement sécurisé', desc: 'Stripe gère toute la sécurité' },
                  { emoji: '🌍', title: '30+ langues', desc: 'Page voyageur multilingue' },
                ].map((item, i) => (
                  <div key={i} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px' }}>
                    <p style={{ margin: '0 0 4px', fontSize: '20px' }}>{item.emoji}</p>
                    <p style={{ margin: '0 0 3px', fontSize: '13px', fontWeight: 800, color: '#1a2a6c' }}>{item.title}</p>
            ...
            {/* Lien vers gestion upsells si actif */}
            {connectStatus === 'active' && (
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                <div>
                  <p style={{ margin: '0 0 3px', fontWeight: 800, color: '#15803d', fontSize: '14px' }}>✅ Compte actif — Upsells disponibles</p>
                  <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>Configurez vos services depuis chaque logement dans le dashboard.</p>
                </div>
                <Link href="/dashboard" legacyBehavior>
                  <a style={{ background: '#15803d', color: 'white', padding: '10px 18px', borderRadius: '10px', fontWeight: 700, fontSize: '13px', whiteSpace: 'nowrap' }}>
                    Gérer mes upsells →
                  </a>
                </Link>
              </div>
          ...
          <div className="settings-card">
                    ...
                    <div key={member.id} className="member-card">
                      <div className="member-info">
                        ...
                        <div className="member-meta">
                          <span className="role-badge" style={{ color: badge.color, background: badge.bg, border: `1px solid ${badge.border}` }}>{badge.label}</span>
                      ...
                      <div className="member-actions">
                        {member.status === 'active' && (
                          <select className="role-select" value={member.role} onChange={e => handleRoleChange(member.id, e.target.value)}>
                            <option value="manager">Gestionnaire</option>
                            <option value="operator">Opérateur d'urgence</option>
                          </select>
                        )}
                        <button className="btn-danger" onClick={() => handleRemoveMember(member.id)}>Retirer</button>
                      </div>
                    </div>
                  );
                })}
            ...
            {!showInviteForm ? (
              <button className="btn-invite" onClick={() => setShowInviteForm(true)}>➕ Inviter un membre</button>
            ) : (
              <div className="invite-form">
                <p className="invite-form-title">✉️ Nouvelle invitation</p>
                {inviteSuccess ? (
                  <div className="toast-success">✅ Invitation envoyée !</div>
                ) : (
                  <>
                    <div className="invite-grid">
                      <div className="invite-field" style={{ gridColumn: 'span 2' }}>
                        <label>Email du membre</label>
                        <input type="email" placeholder="gestionnaire@example.com" value={inviteData.email} onChange={e => setInviteData(prev => ({ ...prev, email: e.target.value }))} />
                      </div>
                      <div className="invite-field">
                        <label>Rôle</label>
                        <select value={inviteData.role} onChange={e => setInviteData(prev => ({ ...prev, role: e.target.value }))}>
                          <option value="manager">🔧 Gestionnaire</option>
                          <option value="operator">🔔 Opérateur d'urgence</option>
                        </select>
                      </div>
                      <div className="invite-field">
                        <label>Accès</label>
                        <select value={inviteData.property_ids === null ? 'all' : 'select'} onChange={e => setInviteData(prev => ({ ...prev, property_ids: e.target.value === 'all' ? null : [] }))}>
                    ...
                    {inviteData.property_ids !== null && properties.length > 0 && (
                      <div style={{ marginBottom: '16px' }}>
                        <p style={{ fontSize: '12px', color: '#0369a1', fontWeight: 700, marginBottom: '8px', textTransform: 'uppercase' }}>Logements assignés</p>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          {properties.map(prop => (
                            <label key={prop.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontSize: '14px', color: '#1e293b', textTransform: 'none', letterSpacing: 0 }}>
                              <input type="checkbox" style={{ width: '16px', height: '16px' }} checked={(inviteData.property_ids || []).includes(prop.id)}
                                onChange={e => {
                                  const current = inviteData.property_ids || [];
                                  const updated = e.target.checked ? [...current, prop.id] : current.filter(id => id !== prop.id);
                    ...
                    <p className="invite-scope">
                      {inviteData.role === 'operator' ? "🔔 Cette personne recevra uniquement les alertes pour les logements assignés." : "🔧 Cette personne pourra gérer les logements assignés."}
                    </p>
                    <div className="invite-actions">
                      <button className="btn-cancel-invite" onClick={() => { setShowInviteForm(false); setInviteData({ email: '', role: 'manager', property_ids: null }); }}>Annuler</button>
                      <button className="btn-invite" onClick={handleInvite} disabled={inviting || !inviteData.email.trim()} style={{ opacity: inviting || !inviteData.email.trim() ? 0.6 : 1 }}>
          ...
          <div className="settings-card">
            ...
            <div className="plan-box">
              <div className="plan-info">
                <h3>{profile.active_licenses > 0 ? `${profile.active_licenses} Logement${profile.active_licenses > 1 ? 's' : ''} Actif${profile.active_licenses > 1 ? 's' : ''}` : 'Aucun logement actif'}</h3>
                <p>Statut : <b>{profile.subscription_status || 'Inactif'}</b>{profile.active_licenses > 0 && <span> · {(profile.active_licenses * 9.9).toFixed(2)}€/mois</span>}</p>
              </div>
              {profile.active_licenses > 0 ? <span className="badge-active">✓ Actif</span> : <span className="badge-inactive">Inactif</span>}
            </div>
            <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              {profile.active_licenses > 0 ? (
                ...
                <Link href="/pricing" legacyBehavior><a className="btn btn-primary">🎩 Activer un logement — 9,90€ le 1er mois</a></Link>
              )}
            </div>
          </div>
        </div>
