...
export default function Register() {
  ...
  return (
    <div className="container">
      <Head>
        <title>Create an account — Alfred Major | AI Butler for Rentals</title>
        <meta name="description" content="Create your Alfred Major account for free. Set up your AI butler in 5 minutes. First month free, no commitment." />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://www.alfredmajor.com/en/register" />
        ...
        <meta property="og:title" content="Create an account — Alfred Major" />
      ...
      <div className="card">
        <div className="card-title">{inviteId ? 'Join the team' : 'Create an account'}</div>
        <div className="card-sub">{inviteId ? 'Create your account to accept the invitation.' : 'Join the hosts who trust Alfred.'}</div>

        {inviteId && inviteReady && (
          <div className="invite-banner">
            <div className="invite-banner-title">✉️ Alfred Major Invitation</div>
            <div className="invite-banner-sub">Create your account below to accept.</div>
          </div>
        )}
        ...
        <form onSubmit={handleRegister}>
          ...
          <input type="text" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Your first name" autoCapitalize="words" />
          <label>Email</label>
          <input type="email" value={email} onChange={e => !inviteId && setEmail(e.target.value)} placeholder="your@email.com" autoCapitalize="none" readOnly={!!inviteId} />
          <label>Password</label>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="8 characters minimum" />
          <button type="submit" className="btn" disabled={loading}>
            {loading ? 'Creating...' : inviteId ? '✅ Create account & Join' : 'Create my account'}
          </button>
        </form>

        <div className="footer-link">
          Already have an account? <Link href={inviteId ? `/en/login?invite=${inviteId}&email=${encodeURIComponent(email)}` : '/en/login'}>Log in</Link>
        </div>
      </div>
    </div>
  );
