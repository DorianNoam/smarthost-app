...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (authError) {
      setError('Invalid credentials or unconfirmed account.');
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="container">
      <Head>
        <title>Log in — Alfred Major | Host Dashboard</title>
        <meta name="description" content="Log in to your Alfred Major host dashboard to manage your properties and AI butler." />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://www.alfredmajor.com/en/login" />
        ...
        <meta property="og:title" content="Log in — Alfred Major" />
      ...
      <div className="card">
        <div className="card-title">{inviteId ? 'Join the team' : 'Log in'}</div>
        <div className="card-sub">{inviteId ? 'Sign in to accept the invitation.' : 'Welcome back.'}</div>

        {inviteId && (
          <div className="invite-banner">
            <div className="invite-banner-title">✉️ Pending invitation</div>
            <div className="invite-banner-sub">Sign in to access your team dashboard.</div>
          </div>
        )}
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" className="btn" disabled={loading}>
            {loading ? 'Signing in...' : inviteId ? '✅ Sign in & Join' : 'Log in'}
          </button>
        </form>

        <div className="footer-link">
          No account yet? <Link href={inviteId ? `/en/register?invite=${inviteId}&email=${encodeURIComponent(email)}` : '/en/register'}>Sign up</Link>
        </div>
      </div>
    </div>
  );
