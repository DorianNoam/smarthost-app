...
function getAlertTexts(lang) {
  const texts = {
    fr: { header: '🚨 *ALERTE ALFRED MAJOR*', property: 'Logement', message: 'Message du client', translation: 'Traduction', action: 'Merci de contacter votre client dans les plus brefs délais pour gérer cette urgence.', regards: 'Cordialement,', signature: "L'équipe Alfred Major", pushTitle: '🚨 Alfred Major — Urgence détectée', translateTo: 'français' },
    it: { header: '🚨 *ALLERTA ALFRED MAJOR*', property: 'Alloggio', message: 'Messaggio del cliente', translation: 'Traduzione', action: 'Si prega di contattare il cliente il prima possibile per gestire questa emergenza.', regards: 'Cordiali saluti,', signature: 'Il team Alfred Major', pushTitle: '🚨 Alfred Major — Emergenza rilevata', translateTo: 'italiano' },
    en: { header: '🚨 *ALFRED MAJOR ALERT*', property: 'Property', message: 'Guest message', translation: 'Translation', action: 'Please contact your guest as soon as possible to handle this emergency.', regards: 'Kind regards,', signature: 'The Alfred Major team', pushTitle: '🚨 Alfred Major — Emergency detected', translateTo: 'English' },
    es: { header: '🚨 *ALERTA ALFRED MAJOR*', property: 'Alojamiento', message: 'Mensaje del cliente', translation: 'Traducción', action: 'Por favor contacte a su cliente lo antes posible para gestionar esta emergencia.', regards: 'Atentamente,', signature: 'El equipo Alfred Major', pushTitle: '🚨 Alfred Major — Emergencia detectada', translateTo: 'español' },
    de: { header: '🚨 *ALFRED MAJOR ALARM*', property: 'Unterkunft', message: 'Nachricht des Gastes', translation: 'Übersetzung', action: 'Bitte kontaktieren Sie Ihren Gast so schnell wie möglich.', regards: 'Mit freundlichen Grüßen,', signature: 'Das Alfred Major Team', pushTitle: '🚨 Alfred Major — Notfall erkannt', translateTo: 'Deutsch' },
    pt: { header: '🚨 *ALERTA ALFRED MAJOR*', property: 'Alojamento', message: 'Mensagem do cliente', translation: 'Tradução', action: 'Por favor contacte o seu cliente o mais rapidamente possível.', regards: 'Atenciosamente,', signature: 'A equipa Alfred Major', pushTitle: '🚨 Alfred Major — Emergência detectada', translateTo: 'português' },
    nl: { header: '🚨 *ALFRED MAJOR WAARSCHUWING*', property: 'Accommodatie', message: 'Bericht van gast', translation: 'Vertaling', action: 'Neem zo snel mogelijk contact op met uw gast.', regards: 'Met vriendelijke groet,', signature: 'Het Alfred Major team', pushTitle: '🚨 Alfred Major — Noodgeval gedetecteerd', translateTo: 'Nederlands' },
    ar: { header: '🚨 *تنبيه ألفريد ماجور*', property: 'العقار', message: 'رسالة الضيف', translation: 'ترجمة', action: 'يرجى التواصل مع ضيفك في أقرب وقت ممكن.', regards: 'مع التحية،', signature: 'فريق ألفريد ماجور', pushTitle: '🚨 ألفريد ماجور — تم الكشف عن حالة طوارئ', translateTo: 'العربية' },
  };
...
async function getGoogleLocalData(category, fullAddress, propertyType = 'apartment') {
  ...
  try {
    ...
    const placesRes = await fetch('https://places.googleapis.com/v1/places:searchNearby', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Goog-Api-Key': apiKey, 'X-Goog-FieldMask': 'places.displayName,places.shortFormattedAddress,places.primaryType,places.rating' },
      body: JSON.stringify({ includedTypes: typeMap[category] || ["point_of_interest"], locationRestriction: { circle: { center: { latitude: lat, longitude: lng }, radius } }, languageCode: 'fr', maxResultCount: 8 }),
    });
...
// ⚠️ MODIFIÉ : utilise supabaseAdmin
async function sendTelegramAlert(originalMsg, translatedMsg, propertyData) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token) { console.error('⚠️ TELEGRAM_BOT_TOKEN absent'); return; }
  ...
  const { data: profile, error: profileError } = await supabaseAdmin
    ...
    .single();
  if (profileError) console.error('Erreur profile (Telegram):', profileError);
  ...
  if (teamError) console.error('Erreur team_members (Telegram):', teamError);

  console.log(`[Telegram] chat_id propriétaire: ${profile?.telegram_chat_id || 'AUCUN'}, team: ${teamMembers?.length || 0}`);

  if (!profile?.telegram_chat_id && (!teamMembers || teamMembers.length === 0)) {
    console.warn('[Telegram] Aucun destinataire — alerte annulée');
    return;
  }
  ...
  const sendToRecipient = async (chatId, lang) => {
    ...
    return fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      .then(r => r.json())
      .then(data => {
        if (!data.ok) console.error(`Erreur Telegram ${chatId}:`, data);
        else console.log(`[Telegram] OK envoyé à ${chatId}`);
        return data;
      })
      .catch(e => console.error(`Erreur réseau Telegram ${chatId}:`, e));
  };

  const sendPromises = [];
  if (profile?.telegram_chat_id) sendPromises.push(sendToRecipient(profile.telegram_chat_id, profile.preferred_language));
  if (teamMembers) {
...
async function sendPushAlert(originalMsg, translatedMsg, propertyData, targetPropertyId) {
  ...
  try {
    const { data: profile, error: profileError } = await supabaseAdmin
      ...
      .eq('id', propertyData.owner_id)
      .single();
    if (profileError) console.error('Erreur profile (Push):', profileError);
    ...
    if (teamError) console.error('Erreur team_members (Push):', teamError);

    const bodyText = translatedMsg ? `${propertyData.name} : "${translatedMsg}"` : `${propertyData.name} : "${originalMsg}"`;
    const promises = [];

    if (profile) {
      const t = getAlertTexts(profile.preferred_language || 'fr');
      if (profile.push_subscription) {
        promises.push(fetch(`${siteUrl}/api/push-send`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: profile.id, title: t.pushTitle, body: bodyText, url: '/dashboard', urgent: true, propertyName: propertyData.name, lang: profile.preferred_language || 'fr' }),
        }));
      }
      if (profile.expo_push_token) {
        promises.push(fetch('https://exp.host/--/api/v2/push/send', {
          method: 'POST', headers: { 'Accept': 'application/json', 'Accept-encoding': 'gzip, deflate', 'Content-Type': 'application/json' },
          body: JSON.stringify({ to: profile.expo_push_token, sound: 'default', title: t.pushTitle, body: bodyText, priority: 'high', data: { url: '/dashboard', propertyId: targetPropertyId, propertyName: propertyData.name } }),
        }));
      }
    }
    if (teamMembers) {
      for (const member of teamMembers) {
        ...
        if (hasAccess && member.expo_push_token) {
          ...
          promises.push(fetch('https://exp.host/--/api/v2/push/send', {
            method: 'POST', headers: { 'Accept': 'application/json', 'Accept-encoding': 'gzip, deflate', 'Content-Type': 'application/json' },
            body: JSON.stringify({ to: member.expo_push_token, sound: 'default', title: t.pushTitle, body: bodyText, priority: 'high', data: { url: '/dashboard', propertyId: targetPropertyId, propertyName: propertyData.name } }),
          }));
        }
      }
    }
...
function detectUpsellIntent(msg) {
  const m = msg.toLowerCase();
  if (['partir plus tard', 'départ tardif', 'check-out tardif', 'late check', 'rester plus longtemps', 'prolonger', 'midi', 'après 11', 'apres 11', 'quitter plus tard', 'sortir plus tard'].some(k => m.includes(k))) return 'late_checkout';
  if (['arriver tôt', 'arriver tot', 'arriver plus tôt', 'arriver plus tot', 'early check', 'check-in tôt', 'avant 15', 'avant 14', 'avant 13', 'tôt le matin', 'tot le matin', 'entrée anticipée'].some(k => m.includes(k))) return 'early_checkin';
  if (['anniversaire', 'romantique', 'surprise', 'champagne', 'bouteille', 'bouquet', 'fleurs', 'saint-valentin', 'lune de miel', 'honeymoon', 'fête', 'célébration', 'celebration', 'mariage', 'pétales'].some(k => m.includes(k))) return 'romantic';
  if (['ménage pendant', 'nettoyage pendant', 'femme de ménage', 'nettoyage mi', 'ménage supplémentaire', 'chambre propre', 'draps changés', 'linge changé', 'repassage', 'aspirateur'].some(k => m.includes(k))) return 'mid_stay_cleaning';
  if (['navette', 'transfer', 'aéroport', 'airport', 'gare', 'taxi privé', 'voiture avec chauffeur', 'vtc', 'aller chercher'].some(k => m.includes(k))) return 'transfer';
  if (['lit bébé', 'lit parapluie', 'chaise haute', 'baby', 'bébé', 'nourrisson', 'poussette', 'équipement enfant'].some(k => m.includes(k))) return 'baby';
  return null;
}
...
function buildUpsellsSection(upsells, siteUrl, propertySlug) {
  ...
  const lines = upsells.map(u => `- ${u.emoji || '✨'} **${u.name}** — ${u.price}€${u.description ? ` (${u.description})` : ''} → Lien : ${upsellsUrl}?upsell=${u.id}`).join('\n');
...
function detectGuestLocation(msg) {
  ...
  const placeKeywords = ['rue','avenue','boulevard','bd','place','square','street','st.','st ','road','rd ','plaza','calle','via','piazza','straße','strasse','platz','allée','allee','chemin','impasse','quai','cours','route','gare','station','aéroport','airport','métro','metro','arrêt','arret','devant le','devant la','devant l','parc','park','jardin','church','église','eglise','hotel','hôtel','mairie','cathédrale','cathedral','museum','musée','musee','tour ','tower','palais','palace','pont ','bridge'];
...
async function getWalkingDistance(originText, destinationAddress, city, langCode = 'fr') {
  ...
  const biasedOrigin = city && !originText.toLowerCase().includes(city.toLowerCase()) ? `${originText}, ${city}` : originText;
  try {
    const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(biasedOrigin)}&destinations=${encodeURIComponent(destinationAddress)}&mode=walking&language=${langCode}&key=${apiKey}`;
    ...
    let modeAdvice;
    if (distanceMeters <= 1500) modeAdvice = 'walk_only';
    else if (distanceMeters <= 3000) modeAdvice = 'walk_or_transit';
    else modeAdvice = 'transit_or_taxi';
    const directionsUrl = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(biasedOrigin)}&destination=${encodeURIComponent(destinationAddress)}&travelmode=walking`;
    return { origin: biasedOrigin, distanceMeters, durationSeconds: element.duration.value, distanceText: element.distance.text, durationText: element.duration.text, modeAdvice, directionsUrl };
  } catch (e) { console.error('Distance Matrix error:', e); return null; }
}
...
function buildWalkingSection(walkingInfo) {
  ...
  switch (walkingInfo.modeAdvice) {
    case 'walk_only': advice = `RECOMMANDATION OBLIGATOIRE : La distance est courte (${walkingInfo.distanceText}). Recommande UNIQUEMENT la marche à pied. NE SUGGÈRE JAMAIS métro, bus, taxi ou VTC pour cette distance.`; break;
    case 'walk_or_transit': advice = `RECOMMANDATION : Distance modérée (${walkingInfo.distanceText}). La marche reste agréable, tu peux mentionner les transports en commun en option.`; break;
    default: advice = `RECOMMANDATION : Distance importante (${walkingInfo.distanceText}). Suggère les transports en commun, un taxi ou un VTC.`;
  }
...
RÈGLE ABSOLUE : Utilise EXACTEMENT ces données. N'invente JAMAIS une distance ou un temps de trajet. Si le voyageur demande un lien, fournis-le en markdown : [Itinéraire à pied](${walkingInfo.directionsUrl}).`;
}

function isEmergency(msg) {
  const m = msg.toLowerCase();
  return ['fuite','leak','inondation','flood','feu','fire','incendie','panne','coupure','électricité','electricity','blessé','injured','accident','urgence','emergency','danger','cassé','ne fonctionne pas','not working','bloqué','locked out','gaz','gas','odeur','smell','fumée','smoke','porte bloquée','porte fermée'].some(k => m.includes(k));
}
...
function buildTypePersonality(propertyType, propertyName, city) {
  switch (propertyType) {
    case 'riad': return `Tu es Alfred, le majordome personnel du "${propertyName}" à ${city}.
Tu gères un riad / une maison d'hôtes. Utilise le vocabulaire approprié : "votre chambre", "notre établissement", "la réception", "le patio", "nos équipes sur place". Ne dis jamais "votre appartement" ou "votre logement".`;
    case 'bnb': return `Tu es Alfred, le majordome personnel de "${propertyName}" à ${city}.
Tu gères une chambre d'hôtes / B&B. Vocabulaire : "votre chambre", "notre maison", "votre hôte", "la salle du petit-déjeuner". Chaleureux et familial.`;
    case 'private_room': return `Tu es Alfred, le majordome personnel de "${propertyName}" à ${city}.
Tu gères une chambre privée dans un logement partagé. Vocabulaire : "votre chambre", "les espaces partagés", "votre hôte". Respectueux des règles de cohabitation.`;
    case 'gite': return `Tu es Alfred, le majordome personnel de "${propertyName}" à ${city}.
Tu gères un gîte / une villa. Vocabulaire : "votre gîte", "la propriété", "le terrain". Si le voyageur demande des commerces ou restaurants, précise les distances car la zone peut être rurale.`;
    default: return `Tu es Alfred, le majordome personnel de "${propertyName}" à ${city}.`;
  }
}
...
function isRateLimited(ip) {
  ...
  if (now - entry.start > RATE_LIMIT_WINDOW) { rateLimitMap.set(ip, { count: 1, start: now }); return false; }
...
export default async function handler(req, res) {
  ...
  if (isRateLimited(ip)) return res.status(429).json({ answer: "Trop de messages. Merci de patienter quelques instants." });
  const { messagesHistory, propertyId, userLanguage } = req.body;
  if (!messagesHistory || !Array.isArray(messagesHistory) || !propertyId) return res.status(400).json({ answer: "Données manquantes." });
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRegex.test(propertyId)) return res.status(400).json({ answer: "Identifiant invalide." });
  if (messagesHistory.length > 50) return res.status(400).json({ answer: "Historique trop long." });
  const lastUserMsg = messagesHistory[messagesHistory.length - 1]?.text || "";
  if (lastUserMsg.length > 2000) return res.status(400).json({ answer: "Message trop long." });

  const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });
  const langCode = userLanguage ? userLanguage.split('-')[0] : 'fr';

  try {
    // Lecture properties : RLS publique sur is_active=true (laissée avec supabase standard)
    const { data: propertyData, error: propError } = await supabase
      .from('properties').select('*').eq('id', propertyId).eq('is_active', true).single();
    if (propError || !propertyData) return res.status(404).json({ answer: "Logement introuvable ou inactif." });

    // ⚠️ Lecture profile (guard subscription) : supabaseAdmin pour bypass RLS
    const { data: ownerProfileGuard, error: guardError } = await supabaseAdmin
      .from('profiles').select('subscription_status').eq('id', propertyData.owner_id).single();
    if (guardError) console.error('Erreur profile guard:', guardError);

    if (ownerProfileGuard && (ownerProfileGuard.subscription_status === 'paused' || ownerProfileGuard.subscription_status === 'cancelled')) {
      return res.status(200).json({ answer: "Le service de conciergerie est momentanément indisponible. Merci de contacter directement votre hôte." });
    }
    ...
    const { data: activeUpsells } = await supabaseAdmin
      .from('upsells').select('id, name, description, emoji, price, category')
      .eq('property_id', targetPropertyId).eq('is_active', true).order('category');

    const { data: kb } = await supabaseAdmin
      .from('knowledge_base').select('category, content').eq('property_id', targetPropertyId);
    const formattedKB = kb?.map(item => `[${item.category}] : ${item.content}`).join('\n') || "";

    const hostLocalInfo = [propertyData.transport_info, propertyData.local_shops, propertyData.recommendations, propertyData.pharmacy_info].filter(Boolean).join('\n');

    const localCategory = detectLocalCategory(lastUserMsg);
    let googleData = "";
    if (localCategory) googleData = await getGoogleLocalData(localCategory, fullAddress, propertyType);
    ...
    if (!guestOrigin) {
      ...
      if (followUpPattern.test(lastUserMsg)) {
        const userMessages = messagesHistory.filter(m => m.role !== 'alfred' && m.role !== 'marc' && m.role !== 'assistant').slice(-4, -1).reverse();
    ...
    if (guestOrigin && fullAddress) walkingInfo = await getWalkingDistance(guestOrigin, fullAddress, city, langCode);
    const walkingSection = buildWalkingSection(walkingInfo);

    const upsellIntent = detectUpsellIntent(lastUserMsg);
    const matchingUpsells = upsellIntent ? findMatchingUpsells(activeUpsells, upsellIntent) : [];
    ...
    const localSection = hostLocalInfo ? `${hostLocalInfo}${googleData ? `\n\nComplément Google Maps :\n${googleData}` : ''}` : googleData || "";
...
- Personnalise tes réponses : utilise "votre chambre", "votre séjour", "à deux pas de chez vous".
...
- UN lieu par bloc, avec une LIGNE VIDE entre chaque recommandation.
...
- Si une adresse n'est PAS disponible, NE METS PAS la ligne 📍. Ne JAMAIS inventer d'adresse.
...
- Seuls éléments autorisés : emojis, gras avec **texte**, retours à la ligne, et liens markdown [texte](url) UNIQUEMENT pour les itinéraires Google Maps fournis dans la section POSITION ACTUELLE DU VOYAGEUR.

━━━ INFORMATIONS DU LOGEMENT ━━━
(Ces informations sont CONFIDENTIELLES. Voir RÈGLE 1 ci-dessous.)
...
Ne divulgue JAMAIS spontanément les informations suivantes si le voyageur ne les demande PAS explicitement :
- Code de boîte à clés / code d'accès / code porte / code immeuble
- Mot de passe WiFi
- Adresse précise et complète du logement
- Code du portail, parking, garage, tableau électrique
- Localisation de la vanne d'eau ou du disjoncteur (sauf urgence avérée)
- Tout numéro, code, mot de passe ou information d'accès

RÈGLE : Le voyageur DOIT formuler une question EXPLICITE sur l'accès / les codes / le wifi pour que tu divulgues.

2. INFO TECHNIQUE MANQUANTE : Si une info demandée (wifi, code, parking...) est "Non renseigné", dis exactement : "Je n'ai pas cette information pour le moment, je contacte votre hôte."

3. QUARTIER & LOCAL : Pour transports, restaurants, commerces, pharmacies — utilise la section QUARTIER & ENVIRONS avec le format défini.

4. INVENTION INTERDITE : Ne jamais inventer une information, une adresse, une distance ou un temps de trajet.
...
${emergencyInstructions}
   IMPORTANT : This phrase must appear EXACTLY as is to trigger alerts.

6. AUCUNE PROMESSE, AUCUN ENGAGEMENT, AUCUN DÉDOMMAGEMENT :
Tu n'as AUCUNE autorité commerciale. Tu n'es JAMAIS le décideur.
INTERDICTIONS ABSOLUES :
- Remboursement, geste commercial, compensation, réduction, avoir
- Service offert (restaurant, bouteille, dîner, taxi, spa)
- Nuit gratuite, prolongation gratuite, upgrade gratuit
- Délai d'intervention chiffré
- Garantie de résolution
- Check-in/check-out dérogatoire gratuit
- Toute décision touchant prix, planning ou logement

EN CAS DE PROBLÈME, PLAINTE OU URGENCE :
   a) Empathie sincère.
   b) Info technique immédiate si elle existe.
   c) Confirme que tu transmets immédiatement à l'hôte.
   d) NE PROMETS RIEN d'autre.

EXEMPLES :
- "Je suis sincèrement désolé pour cette fuite. La vanne d'eau se trouve [emplacement]. Je préviens immédiatement votre hôte."
- "Je comprends votre désagrément concernant le ménage. Je transmets immédiatement à votre hôte."
- "Je suis navré pour cette coupure. Le tableau électrique se trouve [emplacement]. J'alerte votre hôte sans délai."

INTERDITS :
- "Je vous offre un dîner" ❌
- "Vous aurez un remboursement" ❌
- "Un technicien arrive dans une heure" ❌
- "Je vous autorise à rester jusqu'à 14h sans frais" ❌`;
    ...
    const chatResponse = await groq.chat.completions.create({
      ...
      messages: [
        ...
        ...messagesHistory.map(msg => ({ role: msg.role === 'alfred' ? 'assistant' : 'user', content: msg.text || '' })),
      ],
      temperature: 0.5,
      max_tokens: 600,
    });
    ...
    // ⚠️ Upsert conversation : supabaseAdmin pour bypass RLS
    await supabaseAdmin
      .from('conversations')
      .upsert({ property_id: targetPropertyId, history: newHistory, last_message_at: new Date().toISOString() }, { onConflict: 'property_id' });
    ...
    console.log(`[chat.js] shouldAlert=${shouldAlert} | emergency=${isEmergency(lastUserMsg)} | trigger=${triggerPhrases.some(p => responseText.toLowerCase().includes(p))}`);

    if (shouldAlert) {
      if (targetPropertyId) {
        await supabaseAdmin.from('properties').update({ has_emergency: true }).eq('id', targetPropertyId);
      }
      const { data: ownerProfile } = await supabaseAdmin
        .from('profiles').select('preferred_language').eq('id', propertyData.owner_id).single();
      ...
      if (langCode !== hostLang) {
        try {
          const transRes = await groq.chat.completions.create({
            ...
            messages: [
              { role: 'system', content: `Traduis ce message en ${hostAlertTexts.translateTo}. Réponds UNIQUEMENT avec la traduction, sans explication.` },
    ...
    res.status(200).json({ answer: errorMessages[langCode] || errorMessages['en'] });
  }
}
