  ...
  return (
    <div className="container">
      <Head>
        ...
        <link rel="canonical" href={`https://www.alfredmajor.com${locale === 'fr' ? '/pricing' : `/${locale}/pricing`}`} />
        <link rel="alternate" hrefLang="fr" href="https://www.alfredmajor.com/pricing" />
        <link rel="alternate" hrefLang="en" href="https://www.alfredmajor.com/en/pricing" />
        <link rel="alternate" hrefLang="es" href="https://www.alfredmajor.com/es/pricing" />
      ...
      <style jsx>{`
        ...
        nav { display: flex; justify-content: space-between; align-items: center; padding: 15px 5%; position: fixed; top: 0; left: 0; right: 0; background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); z-index: 1000; box-shadow: 0 1px 10px rgba(0,0,0,0.05); box-sizing: border-box; }
        ...
        .lang-btn { background: none; border: 1px solid transparent; padding: 5px 8px; border-radius: 8px; cursor: pointer; font-size: 15px; transition: 0.2s; font-family: inherit; }
        ...
        .price-card { background: white; padding: 50px 40px; border-radius: 24px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.1); text-align: center; border: 2px solid #d4af37; position: relative; width: 100%; max-width: 420px; display: flex; flex-direction: column; }
        .badge-free { position: absolute; top: -18px; left: 50%; transform: translateX(-50%); background: #d4af37; color: #1a2a6c; padding: 8px 24px; border-radius: 30px; font-weight: 800; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 10px 20px rgba(212,175,55,0.3); white-space: nowrap; }
        ...
        .check-icon { background: rgba(212,175,55,0.15); color: #d4af37; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-weight: 800; font-size: 14px; }
        .cta-pricing { background: #1a2a6c; color: white; padding: 18px; border-radius: 16px; font-weight: 700; font-size: 16px; transition: 0.3s; border: none; cursor: pointer; width: 100%; font-family: inherit; }
        .cta-pricing:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 15px 30px rgba(26,42,108,0.2); background: #1e3280; }
        .cta-pricing:disabled { background: #94a3b8; cursor: not-allowed; }
        ...
        @media (max-width: 600px) { .pricing-header { padding: 120px 15px 40px; } .price-card { padding: 40px 25px; } .price { font-size: 48px; } }
      `}</style>

      <nav>
        <Link href="/" locale={locale} className="brand">Alfred<span className="gold">Major</span> 🎩</Link>
        <div className="nav-right">
          ...
          <Link href="/" locale={locale} className="nav-back">{t.nav.back}</Link>
        </div>
      </nav>

      <header className="pricing-header">
        <h1>{p.title}</h1>
        <p className="subtitle">{p.sub}</p>
      </header>

      <div className="pricing-wrapper">
        <div className="price-card">
          <div className="badge-free">{p.promo}</div>
          <div className="plan-name">{p.planName}</div>
          <div className="price">{p.priceMain}<span>{p.pricePeriod}</span></div>
          <div className="billing-cycle">{p.billing}</div>
          ...
          <button className="cta-pricing" onClick={handleCheckout} disabled={loading}>
            {loading ? p.loading : (user ? p.ctaLoggedIn : p.ctaGuest)}
          </button>
          <div className="guarantee">🔒 {p.secure}</div>
        </div>
      </div>
