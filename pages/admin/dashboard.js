...
export default function SuperAdminDashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // MRR : sum des licences actives × 9,90
      ...
      // Nouveaux payants ce mois (passés en active ce mois)
      // On approx via license_events
      ...
      // Churn ce mois (paused_at ce mois)
      const churnThisMonth = pauses.filter(p => p.paused_at >= startOfMonth).length;

      // Taux de conversion trial → payant
      const totalEverTrialed = total; // tous ont commencé en trial
      const conversionRate = totalEverTrialed > 0
        ? Math.round((actifs.length / totalEverTrialed) * 100)
        : 0;
      ...
      // Trials expirés ce mois (passés en pause ce mois)
